...
let navwal = document.getElementById("nav_wallet");
let calculatedBalanceAmount;
if(localStorage.getItem('balance')){
    calculatedBalanceAmount = Math.floor(localStorage.getItem('balance'));
}else{
    calculatedBalanceAmount = localStorage.setItem('balance', 1000)
}
balanceAmount.textContent = calculatedBalanceAmount.toString() + '₹';
navwal.textContent = calculatedBalanceAmount.toString() + '₹';
...
function placeBet() {
    ...
    if ((counter >= randomStop) && !isFlying && (inputBox.value <= calculatedBalanceAmount)) {
        ...
        if (inputBox.value && (inputBox.value <= calculatedBalanceAmount)) {
            ...
            localStorage.setItem('balance', calculatedBalanceAmount);
            balanceAmount.textContent = calculatedBalanceAmount.toFixed(2).toString() + '₹';
            navwal.textContent = calculatedBalanceAmount.toFixed(2).toString() + '₹';
